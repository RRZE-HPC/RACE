...
Traverse::Traverse(Graph *graph_, RACE::dist dist_, int rangeLo_, int rangeHi_, int parentIdx_, int numRoots_, std::vector<std::map<int, std::vector<Range>>> boundaryRange_):graph(graph_),dist(dist_), rangeLo(rangeLo_),rangeHi(rangeHi_),parentIdx(parentIdx_), numRoots(numRoots_), graphSize(graph_->NROW),distFromRoot(NULL),perm(NULL),invPerm(NULL), boundaryRange(boundaryRange_), boundary_bm(NULL), queue(graphSize), levelData(NULL)
{
    colRangeLo = rangeLo;
    colRangeHi = rangeHi;
...
#ifdef RACE_PERMUTE_ON_FLY
        levelNnz_[curr_dist] += graph->childrenSize[graph->totalPerm[i]];
#else
        levelNnz_[curr_dist] += graph->childrenSize[i];
#endif
