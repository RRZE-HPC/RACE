...
We evaluate the performance of the \acrshort{SymmSpMV} based on parallelization and reordering performed by \acrshort{RACE} and compare it with the two MC approaches introduced above, \CAcomm{and} the \acrshort{MKL} \sout{a specific sparse data format (RSB) which allows for parallelization of \acrshort{SymmSpMV}}. \CAcomm{We are not doing it right???} 
...
On both architectures a characteristic drop in performance levels is encountered around the \texttt{Flan\_1565} and \texttt{G3\_circuit} matrices, where the aggregate size of the two vectors (25 MB) approaches the available LLC sizes. For smaller matrices we have a higher chance that the vectors stay in the cache during the \acrshort{SymmSpMV}, i.e. the vectors must only be transferred \CAcomm{Is it better to use loaded once, else write allocate of LHS vector means two transfers for that vector} once between main memory and the processor for every kernel invocation. 
