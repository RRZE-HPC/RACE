...
Most iterative algorithms use a symmetric operator, so called symmetric Kaczmarz (\SYMMKACZ), where the forward sweep shown in \cref{alg:KACZ} is followed by a backward sweep \ie {\tt row=nrows:-1:1} for the two inner loops in~\cref{alg:KACZ}. The intensity of \SYMMKACZ remains same as of \KACZ. 
