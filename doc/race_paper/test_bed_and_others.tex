...
\begin{table}[htbp]
	...
	\end{center}
\end{table}
 Choosing the matrices 10, 22 and 31, which have approximately the same optimal $\alpha_{\acrshort{SpMV}}$, one can study the delicate influence of matrix structure (i.e. matrix bandwidth and number of rows; see \cref{tab:test_mtx}) and the cache size on the actual data traffic, i.e. the measured values of $\alpha_{\acrshort{SpMV}}$. For matrices which are small enough to allow \atleast partial cache reuse (for \eg \texttt{parabolic\_fem}) the measured $\alpha_{SpMV}$ can be smaller than optimal, in this case we set the measured value to the optimal one.
\CAcomm{problematic, \acrshort{SpMV}, \acrshort{SymmSpMV}}
