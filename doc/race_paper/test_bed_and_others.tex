...
Most iterative algorithms use a symmetric operator, so called symmetric Kaczmarz (\SYMMKACZ), where the forward sweep shown in \cref{alg:KACZ} is followed by a backward sweep \ie {\tt row=nrows:-1:1} {\CA for the two inner loops in~\cref{alg:KACZ}} {\CA I didn't understand this as inner loop will not change, only outer loop needs to be reversed, or did you mean something else}. The intensity of \SYMMKACZ remains same as of \KACZ. 
