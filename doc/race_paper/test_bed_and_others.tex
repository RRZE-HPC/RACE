...
\begin{table}[tbp]
	\footnotesize
	\caption{Technical details (per socket) about the Intel CPUs used for the benchmarks. \CAcomm{Need to run on SKX @ 2.2GHz}}\label{tab:test_bed}
	\begin{center}
		%	\setlength{\tabcolsep}{3em}
		\begin{tabular}{l|cc}
	...
	\end{center}
\end{table} 
As the attainable main memory bandwidth is the input parameter to the \roofline model used later, we have carefully measured this value depending on the data set size for two access patterns (copy and load-only). The data presented in \cref{fig:size_vs_bw} basically show the characteristic performance drop if data set size is too large to fit into the \acrshort{LLC}, which is L3 cache on both architectures (cf. \cref{tab:test_bed} for the actual sizes). 
\begin{figure}[tbp]
	\centering
...
\end{figure}
Interestingly there is no sharp drop at the exact size of the \acrshort{LLC} but a rather steady performance decrease with enhanced data access rates also for data set sizes up to twice the L3 size on \IVB. For \SKX this effect is even more pronounced as the non-inclusive victim L3 architecture only stores data which are not in L2 cache, thus the available cache size for an application may be the aggregate sizes of L2 and L3 caches on this architecture. The final bandwidth for the  \roofline model is chosen as the asymptotic value depicted in \cref{fig:size_vs_bw}. Of course caching effects are extremely sensitive to the data access pattern and thus the value presented here only provide simple upper bound for the \acrshort{SymmSpMV} kernel with its potentially strongly irregular data access. \CAcomm{need to rewrite: with its potentially ...}
...
\end{description}
All code was compiled with the Intel compiler in version 19.0.2 and the following compiler flags: {\tt -fno-alias -xHost -O3} for \IVB and {\tt -fno-alias -xCORE-AVX512 -O3} for \SKX. \sout{We did not enforce AVX-512 wide SIMD instructions on \SKX because this feature is of minor importance for the algorithms discussed here.} \CAcomm{We did}
...
Since bandwidth ($b_s$) depends on the ratio of load to store streams we present the model for both upper (load only) and lower bound (copy) bandwidth case.  
 % Due to the nature of our kernels and matrices having considerable amount of non-zeros our algorithms have considerably more loads compared to stores, therefore we choose $b_s$ to be the most optimistic (load-only) value from \cref{tab:test_bed} if not mentioned otherwise.
