...
Since both kernels are closely related to the \acrshort{SpMV} kernel by structure and computational intensity, we start with a discussion of \acrshort{SpMV} and extend it towards \acrshort{SymmSpMV} and \acrshort{SymmKACZ} later. In all cases the aim is to derive realistic upper performance bounds, which can be estimated once the computational intensity and main memory bandwidth (\acrshort{b_s}, see \cref{tab:test_bed}) is known \cite{Williams_roofline} \ie
 ...
 % Due to the nature of our kernels and matrices having considerable amount of non-zeros our algorithms have considerably more loads compared to stores, therefore we choose $b_s$ to be the most optimistic (load-only) value from \cref{tab:test_bed} if not mentioned otherwise.
   ...
   In the following discussions we choose the \acrshort{CRS} storage format for the serial \CAcomm{why only serial impl.} implementation and assume square matrices.
...
For a given non-zero matrix element ($8~\BYTE + 4~\BYTE$) of the triangular matrix twice the amount of Flops ($4~\FLOP$)  are performed. In addition we have indirect access to the LHS vector (read and write) which increases the $\alpha$ contribution by $3\times$. \sout{The only term left scaling  with \acrshort{SymmNNZR}} \CAcomm{The remaining term scaled with \acrshort{SymmNNZR}} (number of non-zeros per row in upper triangular part of the matrix) is the consecutively accessed row pointer. Please note, the $\alpha$ value for \acrshort{SpMV} and \acrshort{SymmSpMV} may be different (even for the same matrix and the same compute device) as in the latter case the two vectors are accessed irregularly and compete for the same amount of cache. Thus we can assume that the $\alpha$ value measured for \acrshort{SpMV} ($\alpha_\mathrm{\acrshort{SpMV}}$) can be considered as a lower bound for \acrshort{SymmSpMV}. As performance is the product of computational intensity and the main memory bandwidth (see \cref{eq:upper_performance}) this approach provides an upper performance bound for \acrshort{SymmSpMV}.
\begin{comment}
