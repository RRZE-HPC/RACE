...
\begin{algorithm}[htp]
	...
	\begin{algorithmic}[1]
		...
		\STATE {$T\_size\_w$ = $T\_size/w$} 
		\textcolor{gray}{\hspace{0.1em} \% $T\_size\_w$ is the weighted $T\_size$, $w$ is array of weights}
		\textcolor{gray}{\hspace{10em} \% and $w[:] = 1$ for \cref{subsec:LB} }
		\STATE {$mean\_r$ = sum($T\_size\_w[0:num\_sweep:len]$) / $nthreads$}
		\textcolor{gray}{\%mean of red color}
		\STATE {$mean\_b$ = sum($T\_size\_w[1:num\_sweep:len]$) / $nthreads$}
		\textcolor{gray}{\%mean of blue color}
		\STATE {$diff[0:num\_sweep:len]$ = $T\_size\_w[0:num\_sweep:len] .- mean\_r$}
		\STATE {$diff[1:num\_sweep:len]$ = $T\_size\_w[1:num\_sweep:len] .- mean\_b$}
		...
		\ENDIF
		\IF{!$fail$}
		\STATE {$newVar$ = calculate\_variance($T\_ptr$) \textcolor{gray}{\% as seen in \textcolor{darkgray}{Line 5} to \textcolor{darkgray}{Line 11}}}
		\ENDIF
...
\end{algorithm} 
%\lipsum[71]
{\CA TODO: check correctness when weight is employed}
