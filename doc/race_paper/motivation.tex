...
%  \begin{figure}[htbp]
 % 	\centering
  ...
  \end{figure}
   ...
   The basic idea of distance-2 \MCfull is to determine sets of structurally orthogonal
   rows \cite{dist_k_def}, i.e. rows that do not overlap in any column entry and assign a color to each set. \Cref{fig:mc_alpha} shows the original matrix and sets of colors obtained after applying \MC permutation in a symmetric manner. Now each rows within a color can be executed in parallel, but the colors are operated one after the other. Thus a color may contain rows from very different parts of the matrix potentially destroying any locality in the matrix. 
  ...
  Algebraic Block \MCfull (\ABMC) tries to alleviate this problem to an extent by first partitioning the entire matrix to blocks and then applying \MCfull to this blocks. This reduces data traffic as seen in \cref{fig:motivation_data} since within each blocks now we have better data locality.
