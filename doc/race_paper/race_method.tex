...
Thus, we have to modify/replace  {\GW Christie: can you comment on that (???)} our original load balancing scheme which assumed a single thread per \levelGroup. In a first step we assign weights to all levels in a specific stage of the recursion. Assuming that $L_s(i) \subset T_{s-1}(j)$ its weight is defined by
