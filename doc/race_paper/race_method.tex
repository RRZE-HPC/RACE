...
Basically our algorithm tries to reduce the variances of the workload in the level groups, \ie the number of rows ($\acrshort{nrows}(T(i))$) in each \levelGroup $T(i)$. For a given set of \levelGroups we calculate the mean and variance of $\acrshort{nrows}(T(i))$ within each color, \ie red and blue colors.  
       ...
       \begin{figure}[thbp]
	    ...
	    %   		\toprule
	       	...
	       	\begin{tikzpicture}[overlay]
		       	\draw[-, dashed, red, line width=1.5] (-2,0.22) -- (-0.1,0.22);
		       	\draw[-, dashed, red, line width=1.5] (-4.4,-0.4) -- (-0.1,-0.4);
		      	\draw[-, dashed, red, line width=1.5] (-2,-1.05) -- (-0.1,-1.05);
		       	\draw[-, dashed, red, line width=1.5] (-4.4,-1.6) -- (-0.1,-1.6);
		       	\draw[->] (0,0.5) -- (0,-1);
		       	\node[rotate=-90] at (0.25,-0.25) {time};
	       	\end{tikzpicture}
       		\caption{Graph coloring of the \stex for eight threads. Recursion is applied on \levelGroups $T_0(4-7)$ with two threads assigned to each. The parallel execution order is shown on right.  Horizontal red dooted lines indicate synchronization and its extent. Vertical lines distinguish between \levelGroups of different stages (here $T_0$ and $T_1$) which can run in parallel. 
