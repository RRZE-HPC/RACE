...
graph data structure. This is equivalent to
...
First we choose a root vertex and assign it to the
first \level, $L(0)$\@. All other \levels (\acrshort{L_i} for $i > 0$)
are defined to contain vertices that are in neighborhood of vertices
in previous \level $L(i-1)$ but not in \level
$L(i-2)$ \cite{BFS_level_def} \ie
