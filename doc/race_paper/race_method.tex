...
%		\nrowsMath^{total} &= n_r(T_{s-1}(j)) %\text{, i.e. number of vertices in graph}
	...
	Requesting $n_t(T_{-1}(0))=8$ threads for the $T_{-1}(0) = 256$ vertices in \cref{fig:rec_2d-7pt_graph} produces these weights for the initial ($s=0$) levels
	\begin{align*}
