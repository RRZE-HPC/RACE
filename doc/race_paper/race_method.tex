...
%		\nrowsMath^{total} &= n_r(T_{s-1}(j)) %\text{, i.e. number of vertices in graph}
	...
	\end{align*}
	Closeness is controlled by the criterion
	\begin{align*}
	\epsilon &> \epsilon_s, \text{with $\epsilon_s \in  (0,1)$ is a user defined parameter.} 	
	\end{align*}		   
	...
	\item The distribution between adjacent red and blue \levelGroups which are assigned to the same thread(s) as well as the final global load balancing is performed basically using the scheme presented in \cref{subsec:LB}. Here the calculation of mean and variance considers the weights of the levels and \levelGroups {\GW or better: considers the number of threads assigned to each \levelGroup ?}{\CA I wouldn't use levels here, I think the sentence in your comment (blue) is  better $\rightarrow$ Here the calculation of mean and variance considers the number of threads (or weight) assigned to each \levelGroup }. Finally, {\CA after this step} a \levelGroup with weight $w$ will ideally have $w$ times more load (vertices) than a \levelGroup with weight one. Same algorithm \cref{alg:LB} presented in \cref{subsec:LB} can be used except now the weights are the $b$ values instead of ones's {\GW Which "1"s? I can not see them in the algorithm.} {\CA Yes it's not clear, actually one's are there in the comment written in line 6 of \cref{alg:LB}}
	%As explained previously in \cref{subsec:LB} this also takes into account load balancing between different colors (red and blue).
