...
graph data structure. \sout{This is equivalent to} \CAcomm{This we achieve using}
well-known bandwidth reduction algorithms such as \acrfull{RCM}
...
\begin{figure}[tbhp]
	...
	\caption{(a) Levels of the original graph and (b) the permuted
          graph for the \stex. Insets show the corresponding sparsity patterns.
          (c) Shows the entries of the \levelPtr array associated
          with $G'$.}
...
\end{equation}
For undirected graphs as considered in this work $u\xrightarrow{k}v$
also implies $v\xrightarrow{k}u$. Based on this definition we consider
two vertices to be \DK independent if they are not \DK
neighbors. Thus, \levels $L(i)$ and $L(i\pm(k+j))$ of the permuted
...
\end{align*}
which contradicts to $v \in L(i \pm (k+j) ) \forall j \geq 1$. This
implies $u$ and $v$ are \DK independent.
\end{proof}
