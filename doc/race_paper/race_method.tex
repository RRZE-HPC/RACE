...
%In \cref{fig:rec_2d-7pt_graph} we demonstrate the impact of one recursive step applied to our \stex with \DTWO 
       \begin{figure}[H]
	    ...
	    %   		\toprule
	       	   ...
	       	   \bottomrule
	       	\end{tabular}
       		\caption{Graph coloring of the \stex for eight threads. Recursion is applied on \levelGroups $T_0(4-7)$ {\GW Modified that; please check.} with two threads assigned to each. The parallel execution order is shown at top right. Each color is executed by four threads. Horizontal lines indicate synchronisation and its extent. Vertical lines distinguishes between \levelGroups of different stage which can run in parallel. {\GW do i need to synchronize all threads working on orange color? or only those within the original \levelGroup, e.g. $T_0(4)$ ?} }
...
\end{enumerate}
Once this scheme has been performed for a given stage the recursion and this scheme is applied to all all new \levelGroups with more than one thread assigned. This continues until every \levelGroup is assigned to one thread or the maximum recursion level (defined by user) is reached. {\GW Is that correct?}  
