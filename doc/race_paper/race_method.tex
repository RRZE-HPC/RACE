...
one \level between any two \levels (e.g., $L(i) and L(i+2)$) all
...
\levels (e.g., $L(i) and L(i+3)$) we have \DTWO independent
\levels, and so on.
 \begin{figure}[tbp]
 	\centering
...
The weak\GHcomm{definition of ``weak'' comes later on, so I'd drop it
  here} definition used in \cref{corollary_dk} offers many choices for
...
spatial access locality. This idea can be generalized: For \DK
coloring, each \levelGroup contains \atleast $k$ adjacent \levels but
the number of \levels \CAcomm{Was `rows' meant here?}\GHcomm{levels is correct IMO} per
\levelGroup may be different. Thus formed \levelGroups are then \DONE
...
However, choosing the same number of \levels for each \levelGroup may
...
workload for \sout{matrix rows at the top and bottom of the matrix}
\CAcomm{\levelGroups containing the top and bottom rows of the
