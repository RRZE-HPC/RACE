...
The recursive nature of our scheme can be best  described by a tree data structure, which we actually use for internal representation. The data structure reflecting the colored graph in \cref{fig:rec_2d-7pt_graph} including the thread assignments is shown in \cref{fig:rec_2d-7pt_tree}. The root of the tree is our baseline \levelGroup $T_{-1}(0)$ comprising all 256 vertices and all eight threads (having unique $id=0,\ldots,7$). The first level of leaves represents the initial ($s=0$) distribution, with each leaf storing the information of a \levelGroup including its color. Threads are mapped consecutively to the \levelGroups. As can be seen the red $T_0(4)$ \levelGroup which consists of vertices $66,\ldots,90$ (omitting the superscript $0$ for $s=0$) is executed by threads with $id=2,3$.  Applying recursion to $T_0(4)$ this leaf spawns four new leaves at stage $s =1$, i.e. \levelGroups $T_1(0,\ldots,3)$, to be executed by the two threads. Synchronisation only happens between threads having the same root node after executing the same color. 
	 \begin{figure}[thbp]
