...
Keeping in mind the observations from previous \cref{Sec:motivation}, one can observe that it would be best to maintain the non-zeros of matrix close to the diagonal. This was recognized previously for normal sparse matrix computations like \SpMV, and has led to the pre-processing of matrix by applying bandwidth reduction algorithms like ``\RCMfull" (\RCM) \cite{RCM,RCM_Sparse_computation}. Here, we aim to develop a method that does not distort this ideal permutations to a large extent and at the same time resolves \DK dependencies. 
...
\begin{figure}[tbhp]
	\centering
	\subfloat[Example stencil]{\label{fig:2d-7pt-a}\includegraphics[width=0.2\textwidth , height=0.13\textheight]{pics/2d-7pt/stencil.pdf}}
	\hspace{1em}
	\subfloat[Sparsity
...
From \cref{eq:level} one finds that the $i$-th \level consist of all vertices that have a minimum distance of $i$ from the root node. \Cref{alg:BFS} gives an algorithm to find each nodes minimum distance from the root. The total number of levels obtained with this graph traversal will be denoted as \totalLvl. \Cref{fig:2d_7pt_level_construction} shows \levels on the \STEX (\totalLvl = 14), where the main number on each vertex ($v$) refers to the vertex number and the superscript shows the \level number, \ie
