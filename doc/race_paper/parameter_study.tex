...
Finally, we evaluate the scalability of RACE using these two corner cases, the \emph{inline\_1} matrix as well as the \emph{parabolic\_fem} matrix, which is small enough to fit into the cache. 
...
\begin{figure}[tbhp]
	...
	\caption{Scaling results of \acrshort{SymmSpMV} with \acrshort{RACE} compared to \acrshort{SpMV} on one socket of \SKX architecture, for corner case matrices. \CAcomm{Shall I delete SpMV from the plot, since it might confuse readers.}}
