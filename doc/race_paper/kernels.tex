...
intensity value for all matrices is shown in 
\cref{table:alpha_values}. \CAcomm{The measured $\alpha_{\acrshort{SpMV}}$
 value is a good approximation for $\alpha_{\acrshort{SymmSpMV}}$ 
 values (see \cref{sect:SymmSpmv}) in cases where caching effects on 
 vectors do not play a role, therefore the table also presents the 
...
\begin{table}[htbp]
	...
	column. \CAcomm{Move last column to fourth ??} The assumed $\alpha_{\acrshort{SymmSpMV}}$ on \SKX and 
