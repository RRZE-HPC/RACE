...
To match the high levels of parallelism of modern compute devices  we use recursion. The formulation of \acrshort{RACE} allows to simply select a \levelGroup (sub-graph) and apply the three
 ...
 \levelGroup spawns other parallel threads in a nested manner. \CAcomm{Considering space, I think it's better w/o any figures.}
...
on different matrices along with the range of upper performance bounds based on two saturated memory bandwidth measurements (RLM-load and RLM-copy). \CAcomm{Is it better to show only copy?}\GHcomm{No}
