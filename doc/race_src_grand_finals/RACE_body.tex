...
In \cite{feast_mc}, \acrshort{MC} was applied to the Kaczmarz iterative solver {\GW{Referenz!}} which has the same \DTWO dependency as acrshort{SymmSpMV}.
Specifically to \acrshort{SymmSpMV} there has been no previous attempt to use 
graph based techniques {\GW{shouldn't it be MC techniques?}} . General solutions for \acrshort{SymmSpMV} are 
...
As main memory bandwidth is the main performance bottleneck for \acrshort{SymmSpMV} on modern compute devices,
