...
concept of recursion. The formulation of the \acrshort{RACE} method allows
 ...
 within the \levelGroup in a nested manner. \CAcomm{Considering space, I think it's better w/o any figures.}
...
kernel shown in \Cref{alg:symmSpMV}. This allows to have a clear picture of the
...
computation as in practical applications the kernel is executed multiple times making other
...
FEAST\cite{FEAST} is a relatively new numerical algorithm to compute inner eigenvalues.
...
\Cref{fig:feast} shows the measured time and memory footprint of FEAST for 
...
see \Cref{fig:feast}) compared to iterative methods ($\approx \mathcal{O}(n^{1.5})$).
