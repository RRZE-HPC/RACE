...
As a \rDel{yardstick for} \rAdd{measure of} baseline performance we choose the general \acrshort{SpMV} kernel and use the performance model introduced in \Cref{Sec:test_kernels} to quantify the quality of our absolute performance numbers.
...
\rAdd{Balancing by number of non-zeros (\acrshort{nnz}) is used for the load balancing step of \acrshort{RACE} in all performance measurements.}
...
\rDel{Doing successive invocations of the kernel on the same two vectors, however, may lead to unrealistic caching of these vectors if the number of rows is small enough.} 
We use two ring buffers (at least of 50 MB each) holding separate vectors of size \acrshort{nrows} \rAdd{in order to avoid vector data reuse across kernel executions.
This is a pessimistic assumption; its impact may depend on the algorithm that \acrshort{SymmSpMV} is embedded in, its implementation, and the parameters of the matrix. For a quantification, one can use the performance models (\ref{eq:SpMV_intensity}) and (\ref{eq:SymmSpMV_intensity}).}
 After each kernel invocation we switch to the next vector in the two buffers. 
 \rDel{This way we mimic the typical structure of iterative sparse solvers where between successive matrix-vector operations other data intensive kernels are executed, \eg several Level 1 BLAS routines or preconditioning steps.} We run over these two buffers 100 iterations (times) and report the mean performance.
...
\rAdd{Simultaneous multi-threading (SMT) is not used since for most of the matrices in \Cref{tab:test_mtx}, the processor can saturate the main memory bandwidth with one thread per core.}
...
%Here we use it  to understand the
 ...
 larger $\alpha_{\acrshort{SpMV}}$ value. \rDel{Note that} \rAdd{The}  $\alpha_{\acrshort{SpMV}}$ \rAdd{(= assumed $\alpha_{\acrshort{SymmSpMV}}$)}
...
%
 \begin{figure}[t]
 	...
 	\caption{Single core performance of \acrshort{SymmSpMV} \rAdd{on \SKX} 
