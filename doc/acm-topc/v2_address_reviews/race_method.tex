...
\begin{itemize}
	...
	\begin{equation*}
	  N(u) = \set{ v \in V(G) : \rDel{(u,v)}\rAdd{\{u,v\}} \in E(G)}\eos
	...
	\begin{equation*}
		H = (V', \set{ \rDel{(u,v) : (u,v)} \rAdd{\{u,v\}} \in E(G) \text{ and } u,v \in V'})\eos
	\end{equation*}
\end{itemize}
...
\Cref{alg:BFS} \rAdd{(see \Cref{Sec:algo})} shows how to determine this distance and thus set up the
...
Thus, \levels $L(i)$ and $L(i\pm(k+j))$ \rAdd{$\forall j\geq1$} of 
the permuted graph $G'$ are \DK independent \rDel{for all $j\geq1$}, 
denoted as
...
On the other hand, if the matrix contains at least one row \rAdd{that is
significantly longer than the average, parallelism may be very limited 
(e.g., for link matrices like com-Orkut from the SuiteSparse Matrix Collection). For a completely dense row}
\rDel{ there is very limited parallelism as} 
\rAdd{we have $\acrshort{totalLvl}=2$.}} 
\rDel{Note that in} \rAdd{In} all cases, vertices within a single \levelGroup
are computed in their \rDel{original} \rAdd{BFS/RCM permuted} order,
which allows for good spatial locality \rAdd{if the level groups are sufficiently
...
ten level groups leads to the mapping shown in \Cref{fig:2d_7pt_lb}. \rDel{Note that} \rAdd{The}
...
\rDel{Note that the} \rAdd{The} \subgraphs might have ``islands'' (groups of vertices
...
%For \DK the same procedure as \DONE applies, except with a slight difference in selecting the \subgraph. In \DONE we considered \subgraphs induced by \levelGroups, but for \DK coloring this is not sufficient. As seen in \Cref{fig:rec_d2_wrong_a} for \DTWO coloring the selection of $T_0(1)$ as \subgraph and applying the three steps (see \Cref{fig:rec_d2_wrong_b}, \Cref{fig:rec_d2_wrong_c} and \Cref{fig:rec_d2_wrong_d}) did not guarantee \DTWO independence within \levelGroup $T_1$ of the \subgraph, for \eg $T_1(0)$ and $T_1(2)$ are not \DTWO independent (\cf \Cref{fig:rec_d2_wrong_d}). This is due to the fact that for $k>1$ dependencies two vertices $a,b$ within a \subgraph might be connected to a common vertex ($c$) outside the \subgraph leading to a \DK dependency between $a$ and $b$. In \Cref{fig:rec_d2_wrong} we see 	$3 \xrightarrow{1} 1 \text{ \& } 6 \xrightarrow{1} 1 	\implies 3 \xrightarrow{2} 6$, but since vertex $1$ was not in the \subgraph considered we missed this dependency. 
     \begin{figure}[t]
     	...
     	\caption{Two \levelGroups generated by a \DTWO coloring
          ...
          same \rDel{sweep (color)} \rAdd{color} as seen in \Cref{fig:rec_d2_wrong_d}.}
...
now construct the new levels \rAdd{($L_{s+1}(:)$)} on this \subgraph considering the neighborhood, 
but \rAdd{in $L_{s+1}(:)$}
we only store the vertices \rDel{in the new \levels $L_{s+1}(:)$}
 that are in the
...
to \rDel{them} \rAdd{these \levelGroups}. 
...
\item Assign weights to all levels at stage ($s$) of the recursion. Assuming
  that $L_s(i) \subset T_{s-1}(j)$, its weight is defined by
	\begin{align*}
		w(L_s(i)) &=
               %\rDel{\frac{\acrshort{nrows}(L_s(i))}{\frac{\acrshort{nrows}(T_{s-1}(j))}{\acrshort{nthreads}(T_{s-1}(j))}}
              %  = 
...
\rDel{Now} \rAdd{now} the calculation of mean and variance must consider the number of threads
...
\rAdd{static\footnote{\rAdd{The thread assignment is initially done in a static manner,
