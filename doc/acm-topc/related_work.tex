...
by \cite{liu2015spmv} these approaches have drawbacks like missing backward compatibility or frequent conversion costs. \CAcomm{Due to these problems there are only a very few standard libraries like 
\acrshort{MKL}~\cite{MKL} support primitives for efficient \acrshort{SymmSpMV} operation.
However,} resolving the inherent data dependency in the \acrshort{SymmSpMV} kernel using a \DTWO coloring of the underlying undirected graph is another potential way of tackling the problem which has not been investigated so far to the best of our knowledge. 
