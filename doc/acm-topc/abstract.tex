...
The \acrfull{SymmSpMV} is an important building block for a number of
numerical linear algebra kernel operations or other graph traversal
applications. Parallelizing \acrshort{SymmSpMV} on today's multicore
platforms with up to 100 cores is not an easy task since computing the
component of the solution will be in conflict with other computed
components, enforcing synchronization events.  \sout{Most existing
