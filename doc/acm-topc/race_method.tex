...
requested.\footnote{This implies as the number of levels increase 
...
by level aggregation in the load balancing step. In case of the
...
where we required \DK independence between vertices of different levels but not 
...
%		\nrowsMath^{total} &\acrshort{nrows}= n_r(T_{s-1}(j)) %\text{, i.e. number of vertices in graph}
	...
	\begin{align*}
		\epsilon =  1 - abs(a-b), & \text{ where } b= \max(1,[a])\\
				& \text{and } [a] \text{ is the nearest integer of a},
	\end{align*}
