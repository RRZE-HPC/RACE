                ...
                for eff in ${race_effs_parse}; do
                    ...
                    if [[ ${colorType} == "RACE" ]]; then
                        ...
                        KMP_WARNINGS=0 MKL_NUM_THREADS=1 \
                            ...
                            COLOR_DISTANCE=1 RACE_EFFICIENCY=${eff} \
                            taskset -c 0 ${execFolder}/serialSymmGS \
                            -m "${matrixFolder}/${matrix}" -c 1 -t 1  -p FILL \
                            -C ${colorType} ${rcmFlag} -v \
                            -i ${iter} -e ${SERIALerr} > ${tmpFile}
                    else
                       #here pinning via OMP
                        #try to achieve the same error as RACE
                        #and give 2*iterations of RACE as max. iter
                        echo "KMP_WARNINGS=0 MKL_NUM_THREADS=$thread OMP_NUM_THREADS=${thread} OMP_SCHEDULE=static OMP_PROC_BIND=close OMP_PLACES=cores COLOR_DISTANCE=1 RACE_EFFICIENCY=${eff} taskset -c 0-$((thread-1)) ${execFolder}/colorSymmGS -m ${matrixFolder}/${matrix} -c ${thread} -t 1  -p FILL -C ${colorType} ${rcmFlag} -i ${iter} -e ${SERIALerr}" >> ${raw_file}
                        KMP_WARNINGS=0 MKL_NUM_THREADS=$thread \
                            OMP_NUM_THREADS=${thread} OMP_SCHEDULE=static \
                            OMP_PROC_BIND=close OMP_PLACES=cores \
                            COLOR_DISTANCE=1 RACE_EFFICIENCY=${eff} \
