...
matrices=$(cat ${configFile} | grep "matrixList" | cut -d"=" -f2)
...
for matrix in $matrices; do
    ...
    lineNum=$(grep -n "${matrix}" $PARAM_file | cut -d":" -f1)
    line=$(head -n ${lineNum} ${PARAM_file} | tail -n 1)
    ...
    iter=$(echo "${PARAMiter}*4" | bc -l) #scale by 4, to see what happens at 4x iterations

    #allow error to be unique, therefore taken from Serial file
    lineNumSerial=$(grep -n "${matrix}" $PARAM_SERIAL_file | cut -d":" -f1)
    lineSerial=$(head -n ${lineNumSerial} ${PARAM_SERIAL_file} | tail -n 1)
    PARAMerr_w_space=$(echo ${lineSerial} | cut -d"," -f5)
    PARAMerr=$(echo ${PARAMerr_w_space}) #ignored, only iter controls the algorithm
    ...
    for thread in $threads; do
        if [[ ${colorType} == "RACE" ]]; then
            ...
            #try to achieve the same error as SERIAL
        ...
        if [[ ${colorType} == "RACE" ]]; then
            echo "${matrix},${thread},${PARAMrcm},${effPrintStr},${niter},${resNorm},${errNorm},${actualIter},${preTime},${perfRes}" >> ${res_file}
        else
            echo "${matrix},${thread},${PARAMrcm},${niter},${resNorm},${errNorm},${actualIter},${preTime},${perfRes}" >> ${res_file}
        fi
