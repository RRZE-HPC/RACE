...
int main(const int argc, char * argv[])
{
    ...
    sparsemat* origMat = new sparsemat;

    printf("Reading matrix file\n");
    if(!origMat->readFile(param.mat_file))
    {
    ...
    for(auto cacheIt=cacheSizes.begin(); cacheIt!=cacheSizes.end(); ++cacheIt)
    {
        ...
        for(auto powIt=powers.begin(); powIt!=powers.end(); ++powIt)
        {
            ...
            mat->prepareForPower(power, cacheSize, param.cores, param.smt, param.pin);
...
#pragma omp parallel
                ...
                printf("Trad. perf summary: cache size = %f, power = %d, SpMV power perf. = %f GFlop/s, time = %f\n", cacheSize, power, flops/spmvPowerTime, spmvPowerTime);
#ifdef VALIDATE_wo_PERM
                delete xTRAD_perf;
#endif
            }
...
#pragma omp parallel
            ...
            printf("RACE perf summary: cache size = %f, power = %d, RACE power perf. = %f GFlop/s, time = %f s\n", cacheSize, power, flops/RACEPowerTime, RACEPowerTime);
