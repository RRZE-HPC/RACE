...
#define SPMV_KERNEL_BODY()                                                     \
  {                                                                            \
    double tmp = 0;                                                            \
    _Pragma("nounroll")                                                        \
        _Pragma("simd vectorlength(VECTOR_LENGTH) reduction(+:tmp)") for (     \
            int idx = mat->rowPtr[row]; idx < mat->rowPtr[row + 1]; ++idx) {   \
...
#pragma omp parallel for schedule(static)
      for (int part = mat->colorPtr[color]; part < mat->colorPtr[color + 1];
           ++part) {
        for (int row = mat->partPtr[part]; row < mat->partPtr[part + 1];
             ++row) {
          SPMV_KERNEL_BODY();
        }
      }
    ...
    for (int color = 0; color < mat->ncolors; ++color) {
#pragma omp parallel for schedule(static)
      for (int row = mat->colorPtr[color]; row < mat->colorPtr[color + 1];
           ++row) {
        SPMV_KERNEL_BODY();
      }
    }
  } else {
    printf("Didn't recognize the coloring type. Available options: RACE, ABMC, "
           "MC\n");
  }
}
...
#define SPMTV_KERNEL_BODY()                                                    \
  {                                                                            \
    ...
    _Pragma(                                                                   \
        "simd vectorlength(VECTOR_LENGTH)") for (int idx = mat->rowPtr[row];   \
                                                 idx < mat->rowPtr[row + 1];   \
                                                 ++idx) {                      \
...
#define GS_KERNEL_BODY()                                                       \
  {                                                                            \
    ...
    _Pragma("simd vectorlength(VECTOR_LENGTH) reduction(+:tmp)") for (         \
        int idx = mat->rowPtr[row] + 1; idx < mat->rowPtr[row + 1]; ++idx) {   \
      tmp += (-mat->val[idx]) * x->val[mat->col[idx]];                         \
    }                                                                          \
...
void gs(densemat *b, sparsemat *mat, densemat *x, bool rev) {
  if ((mat->colorType == "RACE") || (mat->colorType == "RACE_and_BLOCK")) {
    ...
    for (int color = startColor; color != endColor; color = color + incColor) {
      int startPart =
          (!rev) ? mat->colorPtr[color] : mat->colorPtr[color + 1] - 1;
      int endPart =
          (!rev) ? mat->colorPtr[color + 1] : mat->colorPtr[color] - 1;
      int incPart = (!rev) ? 1 : -1;
...
#define SYMM_SPMV_KERNEL_BODY()                                                \
  {                                                                            \
    ...
    double temp = 0;                                                           \
    _Pragma("simd reduction(+:temp)") /*Not doiing here for vector length      \
                                         because nnzr can be very small, lat   \
                                         compiler decide*/                     \
        for (int idx = mat->rowPtr_symm[row] + 1;                              \
             idx < mat->rowPtr_symm[row + 1]; ++idx) {                         \
...
inline void PLAIN_SPMV_KERNEL_only_highest(int start, int end, int pow,
                                           void *args) {
  DECODE_FROM_VOID(args);
...
#pragma nounroll
#pragma simd vectorlength(VECTOR_LENGTH) reduction(+ : tmp)
    for (int idx = mat->rowPtr[numa_domain][row];
         idx < mat->rowPtr[numa_domain][row + 1]; ++idx) {
      tmp += mat->val[numa_domain][idx] *
             x->val[col_offset + mat->col[numa_domain][idx]];
    }
...
void mkl_spmv(densemat *b, sparsemat *mat, densemat *x, bool symm) {
  int nrows = mat->nrows;
  if (!symm) {
    mkl_cspblas_dcsrgemv("N", &nrows, ((double *)mat->val),
                         ((MKL_INT *)mat->rowPtr), ((MKL_INT *)mat->col),
                         ((double *)x->val), ((double *)b->val));
  } else {
    mkl_cspblas_dcsrsymv(
        "U", &nrows, ((double *)mat->val_symm), ((MKL_INT *)mat->rowPtr_symm),
        ((MKL_INT *)mat->col_symm), ((double *)x->val), ((double *)b->val));
  }
}
...
void mkl_spmv(sparsemat *mat, densemat *x, bool symm) {
  ...
  if (!symm) {
    ...
    mkl_cspblas_dcsrsymv(
        "U", &nrows, ((double *)mat->val_symm), ((MKL_INT *)mat->rowPtr_symm),
        ((MKL_INT *)mat->col_symm), ((double *)x_), ((double *)b_));
  }
}
...
sparse_matrix_t *mkl_ie_setup(sparsemat *mat, int niter, bool symm) {
  ...
  if (!symm) {
    status = mkl_sparse_d_create_csr(
        A_mkl, SPARSE_INDEX_BASE_ZERO, nrows, nrows, &(((int *)mat->rowPtr)[0]),
        &(((int *)mat->rowPtr)[1]), ((int *)mat->col), ((double *)mat->val));
    descr.type = SPARSE_MATRIX_TYPE_GENERAL;
  } else {
    mat->computeSymmData();
    status = mkl_sparse_d_create_csr(
        A_mkl, SPARSE_INDEX_BASE_ZERO, nrows, nrows,
        &(((int *)mat->rowPtr_symm)[0]), &(((int *)mat->rowPtr_symm)[1]),
        ((int *)mat->col_symm), ((double *)mat->val_symm));
  ...
  status = mkl_sparse_set_mv_hint(*A_mkl, SPARSE_OPERATION_NON_TRANSPOSE, descr,
                                  niter);
  if (status != SPARSE_STATUS_SUCCESS) {
...
void mkl_ie_spmv(densemat *b, sparse_matrix_t *A, densemat *x, bool symm) {
  ...
  mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE, 1, *A, descr,
                  ((double *)x->val), 0, ((double *)b->val));
}
...
void mkl_ie_spmv(sparse_matrix_t *A, densemat *x, bool symm) {
  ...
  mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE, 1, *A, descr, ((double *)x_),
                  0, ((double *)b_));
}

void mkl_ie_free(sparse_matrix_t *A) { delete A; }

inline void MAT_SPMV_KERNEL_only_highest(int start, int end, int pow,
...
void matPower_only_highest(sparsemat *A, int power, densemat *x) {
  ...
  int race_power_id =
      ce->registerFunction(&MAT_SPMV_KERNEL_only_highest, voidArg, power);
  {
...
inline void MAT_SPMV_KERNEL(int start, int end, int pow, int numa_domain,
                            void *args) {
  DECODE_FROM_VOID(args);
...
inline void MAT_SPMV_NUMA_KERNEL(int start, int end, int pow, int numa_domain,
                                 void *args) {
  DECODE_FROM_VOID_NUMA(args);
...
void matPowerNuma(NUMAmat *A, int power, densemat *x) {
  ...
  int race_power_id =
      ce->registerFunction(&MAT_SPMV_NUMA_KERNEL, voidArg, power, true);
  {
    ce->executeFunction(race_power_id);
  }
  DELETE_ARG();
...
inline void MAT_SPMV_KERNEL_BCSR_2x2(int start, int end, int pow,
                                     int numa_domain, void *args) {
  DECODE_FROM_VOID(args);
...
#pragma unroll_and_jam(4)
      for (int b_x = 0; b_x < 2; ++b_x) {
        for (int b_y = 0; b_y < 2; ++b_y) {
          tmp[b_x] += mat->val[4 * idx + b_x * 2 + b_y] *
                      x->val[offset + 2 * mat->col[idx] + b_y];
        }
      }
    }
...
void matPowerBCSR(sparsemat *A, int power, densemat *x) {
  ...
  ENCODE_TO_VOID(A, NULL, x);
  int race_power_id =
      ce->registerFunction(&MAT_SPMV_KERNEL_BCSR_2x2, voidArg, power);
  {
    ce->executeFunction(race_power_id);
  }
  DELETE_ARG();
