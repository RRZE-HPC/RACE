...
perf_files_untransposed=`cat $1 | grep PERF_FILES | awk '{ for(i=3;i<=NF;i++) {print $i} }'`
...
mark="default triangle square diamond otimes triangle"

perf_files=""
ctr=1
#transpose all input files
for file in $perf_files_untransposed; do
    sed 's/$/,/' ${file} > trFile_commaEnd_${ctr}.txt
    awk '
    {
        for (i=1; i<=NF; i++)  {
            a[NR,i] = $i
    ...
    }' trFile_commaEnd_${ctr}.txt > trFile_${ctr}.txt
    rm -rf trFile_commaEnd_${ctr}.txt
    perf_files="${perf_files}trFile_${ctr}.txt "
    let ctr=${ctr}+1
done

mtx_name_file=$(echo $perf_files | cut -d " " -f  1)
cat $mtx_name_file | tail -n +2 >temp.txt
cat temp.txt>sorted_file.txt
rm temp.txt
mtx_name_file="sorted_file.txt"


cut -d"," -f$col_mtx_name $mtx_name_file > col0.txt
